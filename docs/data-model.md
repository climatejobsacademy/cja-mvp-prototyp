...
| competency | Eine Kompetenz aus der Kompetenzmatrix | name, kompetenzbereich, quelle (TQ-ARP / EFKffT / EFK-EE; veraltet seit 0020, Programm-Zuordnung über `competency_programme`) | AfCJ | Ja | — |
| competency_programme | N:M: welche Kompetenz gehört zu welchem Qualifizierungsprogramm | competency_id, programme_id | AfCJ | Ja | competency, programme |
| competency_step | Ein Teilschritt, kleinster Nachweisbaustein einer Kompetenz | name, typ (theoretisch / praktisch), nachweistyp (aktuell nur binär) | AfCJ | Ja | — (Kompetenz-Zuordnung über `competency_competency_step`) |
| competency_competency_step | N:M: welcher Teilschritt gehört zu welcher Kompetenz — ein Teilschritt kann auf mehrere Kompetenzen einzahlen | competency_id, competency_step_id | AfCJ | Ja | competency, competency_step |
...
Tabelle `programme` entspricht dem Konzept Qualifikationspfad — ein Qualifizierungsprogramm, dem Kompetenzen und Lernende zugeordnet werden können.

Entschieden 2026-09-23 (SR-66): Kompetenz ↔ Programm ist N:M — eine Kompetenz kann mehreren Qualifizierungsprogrammen zugeordnet sein. Neue Junction-Tabelle `competency_programme` (0020). `competency.quelle` wird als Programm-Zugehörigkeit behandelt; bestehende Zuordnungen wurden per Abgleich `programme.kuerzel = competency.quelle` übernommen (nicht über `programme.name`, der den Langnamen enthält). Kompetenzen mit `quelle = 'TQ-ARP'` bekommen dabei keine Zuordnung, weil es kein Programm mit diesem Kürzel gibt (das TQ-Programm heißt `TQ-EGT`) — ob `TQ-ARP` im CHECK-Constraint ein Relikt oder ein künftiger Fall ist, wird mit dem Team geklärt, sobald die TQ-Kompetenzen angelegt werden; laut Stand 2026-09-23 existieren keine TQ-ARP-Kompetenzen. RLS über das referenzierte Programm (Learner sehen nur Zuordnungen zu `published`-Programmen, analog `content_competency_mapping`/`field_job_type_competency_mapping`), write nur AfCJ admin. Umstellung in zwei Schritten wie bei SR-65: `competency.quelle` bleibt vorerst stehen (weiterhin not null inkl. CHECK) und entfällt mit einer eigenen Drop-Column-Migration.

Entschieden 2026-09-23 (SR-65): Kompetenz ↔ Teilschritt ist N:M statt 1:N — ein Teilschritt kann auf mehrere Kompetenzen einzahlen und zählt im Kompetenzprofil dann in jeder davon mit. Neue Junction-Tabelle `competency_competency_step` (0019), bestehende Zuordnungen aus `competency_step.competency_id` wurden 1:1 übernommen. RLS wie `competency`/`competency_step` (Katalogdaten, read All für alle eingeloggten Rollen, write nur AfCJ admin, kein eigener `access-matrix.md`-Eintrag). Umstellung in zwei Schritten: `competency_step.competency_id` bleibt vorerst stehen (weiterhin not null, wird aber nicht mehr gelesen) und entfällt erst mit einer eigenen Drop-Column-Migration, sobald der Code, der nur noch `competency_competency_step` nutzt, auf prod läuft. Bis dahin gilt: maßgeblich ist ausschließlich `competency_competency_step`.
